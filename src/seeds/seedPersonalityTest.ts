...
const personalitySeedVersion = '1.0';
const personalitySeedTitle = 'Test de personnalité Matcha';
const personalitySeedSummary =
...
type PersonalityQuestionSeedDefinition = {
  code: string;
  text: string;
  dimension: PersonalityDimension;
  options: ReturnType<typeof likert>;
};

async function ensurePersonalityVersionedIndexes() {
  await Promise.all([
    PersonalityQuestion.collection.createIndex(
      { version: 1, code: 1 },
      { unique: true }
    ),
    PersonalityProfile.collection.createIndex(
      { version: 1, key: 1 },
      { unique: true }
    ),
  ]);
...
export const seedPersonalityTest = async () => {
  await ensurePersonalityVersionedIndexes();

  const activeVersion = await PersonalityVersion.findOne({
    isActive: true,
  }).lean();
  const shouldActivateSeedVersion =
    !activeVersion || activeVersion.version === personalitySeedVersion;

  if (shouldActivateSeedVersion) {
    await PersonalityVersion.updateMany(
      { version: { $ne: personalitySeedVersion }, isActive: true },
      { $set: { isActive: false, status: 'archived' } }
    );
  }

  const version = await PersonalityVersion.findOneAndUpdate(
    { version: personalitySeedVersion },
    {
      $set: {
        title: personalitySeedTitle,
        summary: personalitySeedSummary,
        isActive: shouldActivateSeedVersion,
        status: shouldActivateSeedVersion ? 'active' : 'draft',
      },
    },
  ...
  const profiles = [
    ...
    },
    {
      ...
      suggestedSectors: ['Management', 'Entrepreneuriat', 'Gestion de projet'],
    },
    {
      key: 'ENTP',
      label: 'Innovateur',
    ...
    },
    {
      ...
      suggestedSectors: ['Action sociale', 'Ressources humaines', 'Education'],
    },
    {
      key: 'ISTP',
      label: 'Artisan',
  ...
  await PersonalityProfile.bulkWrite(
    profiles.map((profile) => ({
      updateOne: {
        filter: { version: profile.version, key: profile.key },
        update: { $set: profile },
        upsert: true,
      },
    })),
  ...
  const questionDefinitions: PersonalityQuestionSeedDefinition[] = [
    {
      code: 'q1',
  ...
  const questions = questionDefinitions.map((question, index) => ({
    ...question,
    versionId: version._id,
    version: version.version,
    order: index + 1,
  ...
  await PersonalityQuestion.bulkWrite(
    questions.map((question) => ({
      updateOne: {
        filter: { version: question.version, code: question.code },
        update: { $set: question },
        upsert: true,
      },
    })),
  ...
  console.log(
    `✅ Test de personnalité ${version.version} seedé: ${profiles.length} profils, ${questions.length} questions.`
  );
};
